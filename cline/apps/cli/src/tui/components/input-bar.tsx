...
export function InputBar(props: InputBarProps) {
	...
	const textareaRefCallback = useCallback(
		(node: unknown) => {
			...
			inputRef.current = ta;
			if (ta) {
				ta.onSubmit = () => onSubmitRef.current();
	...
	const insertAtomicText = useCallback(
		(text: string) => {
			...
			const start = selection
				...
				: ta.cursorOffset;
			ta.insertText(text);
			ta.extmarks.create({ start, end: start + text.length, virtual: true });
			queueMicrotask(() => {
	...
	return (
		...
		>
			{props.isRunning ? (
				<box width={18}>
					<LetterblackLoader color={accent} />
				</box>
			) : (
				<text fg={accent}><strong>[I]</strong></text>
			)}
