...
| `32_deterministic_runtime_state_machine.md` | IMPLEMENTED_PRE_INTEGRATION | CORE / BLOCKING | `04`, `05`, `07`, `13` | Deterministic mock-runtime hardening substantially implemented, including terminal idempotency (duplicate terminal lifecycle event suppression), guarded transitions, timeout enforcement, and identity isolation. RealLbeWrapper read-only skeleton now exists behind LbeWrapper. Real Agent Wall attachment and feature-specific wall capabilities (memory, provenance, impact analysis, hypothesis/test/verify, task graph, validation strategies, documentation artifacts) remain pending. |
| `33_real_cli_ide_implementation_plan.md` | PLANNED | CORE / SEQUENCING | `31`, `32` | Ordered implementation plan: RealLbeWrapper skeleton exists, real wall attachment and snapshot/event stream mapping remain pending, then governed execution, policy/sandbox, provider loop, IDE review surfaces, persistence, headless mode, background processes, and multi-agent work. |
| `34_autonomous_developer_frontend.md` | PARTIAL | CORE | `32`, `33` | Maps TUI-33 through TUI-40 (autonomous developer frontend features) to current implementation state. Frontend scaffolding exists across all eight features (varying degree). Full completion requires real Agent Wall attachment, authoritative event/snapshot stream, and feature-specific wall capabilities — not another state-machine pass. Documents per-feature runtime gap using Handoff Result Format. |
